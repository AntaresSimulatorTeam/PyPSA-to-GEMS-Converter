...
def analyze_benchmark_study(row_number: int, results_file: Path = None):
    ...
    print(f"  Constraints Ratio (PyPSA/Modeler): {row['number_of_constraints_pypsa'] / row['number_of_constraints_modeler']:.4f}")
    print(f"  PyPSA Variables: {int(row['number_of_variables_pypsa'])}")
    print(f"  Modeler Variables: {int(row['number_of_variables_modeler'])}")
    print(f"  Variables Ratio (PyPSA/Modeler): {row['number_of_variables_pypsa'] / row['number_of_variables_modeler']:.4f}")
