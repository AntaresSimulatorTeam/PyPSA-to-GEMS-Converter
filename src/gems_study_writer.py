...
class GemsStudyWriter:
    ...
    def prepare_xpansion_runnable_study(self, solver_name: str) -> None:
        """
        Write the Antares-Xpansion inputs the launcher GEMS workflow needs on top of the GEMS study.

        The GEMS workflow (problem-generation + Benders) is driven by ``input/optim-config.yml`` and the
        GEMS study itself, so no Antares-Simulator scaffold is generated here. Only an empty
        ``user/expansion/settings.ini`` is written: Antares-Xpansion then weights every Monte-Carlo year
        (one per scenario) equally, which matches the equally-weighted scenarios the converter produces.
        """
        if solver_name.lower() not in {"coin", "xpress"}:
            raise ValueError("Multi-scenario Xpansion studies support only 'coin' and 'xpress' solvers.")

        study_root = self.study_dir / "systems"
        self._write_text_file(study_root / "user" / "expansion" / "settings.ini", "")
