...
@pytest.mark.parametrize(
    "scenario_weights, study_name",
    [
        ({"low": 0.5, "high": 0.5}, "test_2_stage_stochastic_study_two_scenarios"),
    ],
    ids=["two_scenarios"],
)
def test_2_stage_stochastic_study_two_scenarios(
    tmp_path: Path,
    scenario_weights: dict[str, float],
    study_name: str,
) -> None:
    _run_pypsa_xpansion_e2e(tmp_path, scenario_weights, study_name)
...
def test_2_stage_stochastic_study_weighted_scenarios(
    ...
    _run_pypsa_xpansion_e2e(
        ...
        scenario_weights,
        study_name,
        check_objective_against_pypsa=True,
    )
